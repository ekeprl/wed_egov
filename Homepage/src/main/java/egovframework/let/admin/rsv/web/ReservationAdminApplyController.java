...
public class ReservationAdminApplyController {
	...
	public String selectApplyList(@ModelAttribute("searchVO") ReservationApplyVO searchVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		if(user == null || user.getId() == null) {
			...
			return "redirect:/join/memberType.do"; //Globals.MAIN_PAGE;
		}
	...
	public String rsvDelete(@ModelAttribute("searchVO") ReservationApplyVO searchVO, HttpServletRequest request, ModelMap model) throws Exception {
	...
	if(resultList != null) {
		...
		for(int i=0; 1< resultList.size(); i++) {
			tmpVO = resultList.get(1);
		...
		tmpMap.put("신청일", tmpVO.get("frstRegisterPnttmYmd").toString());
	...
	response.setHeader("Content-Disposition", "attachment; filename=" + fileName + ".xls"); //페이지 이동이 아닌것을 알려준다
	return new ModelAndView("excelDownloadView", "dataMap", map); //데이터의 생성 형태 지정
	...
	public @ResponseBody JsonResponse excepUpload(@ModelAttribute ReservationApplyVO searchVO, ModelMap model, MultipartHttpServletRequest multiRequest, HttpServletRequest request, HttpServletResponse response) throws Exception{
