...
<c:url var="_BASE_PARAM" value="">
	...
	<c:if test="${not empty searchVO.searchCondition}">
		<c:param name="searchCondition" value="${searchVO.searchContdition }"/>
	</c:if>
...
<div class="container">
	<div id="contents">
	 ...
	 <div id="bbs_wrap">
	 	<div class="total">
	 		총 게시물
	 		<strong><c:out value="${paginationInfo.totalRecordCount }"/></strong>건 | 현재페이지 <strong><c:out value="${paginationInfo.currentPageNo }"/></strong>/<c:out value="${pagenationInfo.totalPageCount }"/>
	 	</div>
	 	<div class="bss_list">
	 	<table class="list_table">
	 		<thead>
	 			<tr>
	 				...
	 				<th class="writter" scope="col">작성자</th>
 				...
 				<c:forEach var="result" items="${resultList}" varStatus="status">
 					<tr>
 						...
 						<td class="tit">
							...
							<a href="${viewUrl}">
								<c:if test="${result.othbcAt eq 'Y'}"><img src="/asset/BBSTMP_0000000000001/images/ico_board_lock.gif" alt="비밀 글 아이콘"/></c:if>
							...
							<td class="hits" data-cell-header="조회수 :"><c:out value="${result.inqireCo}"/>
							</td>
						</tr>
					</c:forEach> 
