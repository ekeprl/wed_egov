			...
			<div id="contents">
				...
				<div id="bbs_wrap">
							...
							<tbody>
								<c:forEach var="result" items="${resultList}" varStatus="status">
									<tr>
										...
										<td><c:url var="deleteUrl" value="/admin/rsv/rsvApplyDelete.do${_BASE_PARAM}">
												<c:param name="resveId" value="${result.resveId}" />
												<c:param name="reqstId" value="${result.reqstId}" />
												<c:param name="pageIndex" value="${searchVO.pageIndex}" />
											</c:url>
					...
					<div class="excelUploadBox">
						<form id="excelForm" name="excelForm" action="/admin/rsv/excelUpload.json" enctype="multipart/form-date" method="post"> 
																							  <!-- enctype이 없으면 첨부파일이 올라가지 않는다 + 첨부파일은 항상post방식으로 -->
							...
							<br/>
							<label for="registerExcelFile">파일 첨부</label><input type="file" id="registerExcelFile" name="registerExcelFile" class="upload-hidden">
							<a href="#" id="excelReg" class="btn btn_blus">등록</a>						
						</form>
					</div>
				...
				,success : function(result) {
					...
					if(result.success) {
						...
						for(i = 0; i < result.data.length; i++){
							...
							message += result.data[1].userId + " : " + result.data[i].message;
						}						
						alert(message);
						window.location.reload();
					} 
